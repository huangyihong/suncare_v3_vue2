<template>
  <a-card :bordered="false">
          ...
          <template v-if="toggleSearchStatus">
            ...
            <a-col :md="6" :sm="8">
              <a-form-item label="限制类型">
                <m-dict-select-tag  placeholder="请选择限制类型" dictCode="DC_DRUG_LIMIT_TYPE"  v-model="queryParam.limitType"></m-dict-select-tag>
              </a-form-item>
            </a-col>
            <a-col :md="6" :sm="8">
              <a-form-item label="限制内容">
                <j-input placeholder="请输入内容" v-model="queryParam.limitContent"></j-input>
              </a-form-item>
            </a-col>
            <a-col :md="6" :sm="8">
    ...
    <div class="table-operator">
      ...
      <a-button @click="getExamine" v-has="'drugRuleCase:examine'"  type="primary" icon="bulb">批量审核
      </a-button>
...
<!--          </a-dropdown>-->
    ...
    <!-- 表单区域 -->
    <yb-charge-drug-rule-modal ref="modalForm" @ok="modalFormOk"></yb-charge-drug-rule-modal>
    <!--    导入对话框-->
    <import-excle-model ref="importExcleModel" :text="'药品规则库导入'" :importExcel="this.url.importExcelUrl"  @ok="refresh"></import-excle-model>
...
<script>
import YbChargeDrugRuleModal from './modules/YbChargeDrugRuleModal'
...
export default {
  ...
  data() {
    return {
      ...
      num: 0,
      url: {
        list: "/ybChargeSearch/ybChargeDrugRule/list",
        delete: "/ybChargeSearch/ybChargeDrugRule/delete",
        deleteBatch: "/ybChargeSearch/ybChargeDrugRule/deleteBatch",
        exportXlsUrl: "ybChargeSearch/ybChargeDrugRule/exportXls",
        importExcelUrl: "ybChargeSearch/ybChargeDrugRule/importExcel",
        batchExamine: "ybChargeSearch/ybChargeDrugRule/batchExamine"
      },
    }
  },
  computed: {
  ...
  methods: {
    ...
    handleExportExcel() {
      ...
      this.handleExportXls('药品规则库' + moment().format('YYYYMMDDHHmmss'))
    },
    getExamine() {
    ...
    downloadTemplate() {
      window.open(this.$config['domianURL'] + "/sys/common/download/ybDrugRule.xlsx");
    },
    //修改
    update() {
      if (this.selectionRows.length !== 1) {
