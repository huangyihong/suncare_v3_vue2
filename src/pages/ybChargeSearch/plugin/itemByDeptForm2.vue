                ...
                <a-col :md="6" :sm="8">
                  <a-form-item style="margin-bottom:4px;">
                    <a-checkbox-group  @change="selectType">
                      <a-checkbox value="deptStatistics">每个科室就诊金额排名前10的就诊明细下载</a-checkbox>
                    </a-checkbox-group>
                  </a-form-item>
                </a-col>
              </a-row>
...
export default {
  ...
  methods: {
    ...
    clearExcelTable(){
      this.bean = Object.assign({}, {deptname:undefined})
    },
    changeTaskType(key){
      this.$emit('changeTaskType',key);
    },
    setFormData(data,selectOrgsArr){
      this.form.setFieldsValue(pick(data,'orgs','chargedateStartdate','chargedateEnddate','visittype','tagName','etlSource','dataStaticsLevel'))
      this.orgs = data.orgs
      this.selectOrgsArr = selectOrgsArr
      this.datasourceAndDatabaseVO = data.datasourceAndDatabaseVO
      this.$nextTick(() => {
        this.changeVisittype(data.etlSource,this.taskType)
      })
    ...
    getDeptList(orgArr){
      ...
      if(orgArr&&orgArr.length>0){
        ...
        getAction(this.url.getDeptList, {dataSource:this.userInfo.dataSource,etlSource:this.form.getFieldValue('etlSource'), orgs,orgids,taskType:'itemByDeptStatistics'}).then((res) => {
          if (res.success) {
    ...
    handleOk () {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          //HEXG处理科室前10的类型
          if(!this.bean.taskType || this.bean.taskType !='deptStatistics'){
