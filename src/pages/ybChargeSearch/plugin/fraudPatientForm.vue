          ...
          <a-row :gutter="24">
            <a-col :md="6" :sm="8">
              <a-form-item label="患者姓名">
                <a-input placeholder="请输入患者姓名" v-model="bean.clientname"></a-input>
              </a-form-item>
            </a-col>
            <a-col :md="6" :sm="8">
              <a-form-item label="出生年月日">
                <a-date-picker placeholder="请选择出生年月日" v-model="bean.name" :style="{width:'100%'}"></a-date-picker>
              </a-form-item>
            </a-col>
...
export default {
  ...
  data () {
    return {
      description: '欺诈专题患者年度统计',
      bean:{clientname:'',name:''},
    }
  },
  beforeMount() {
    this.$nextTick(() => {
  ...
  methods: {
    clearExcelTable(){
      this.bean = Object.assign({}, {clientname:'',name:''})
    },
    ...
    searchQuery(){
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          if(formData.name){
            formData.name = formData.name.format("YYYY-MM-DD")
          }
