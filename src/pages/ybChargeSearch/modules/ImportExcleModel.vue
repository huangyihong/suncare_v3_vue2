  ...
  <div>
    <a-modal :title="title" :closeIcon="close" :width="420" :closable="false" :visible="visible"
      ...
      <div class="addBox m-t-15" id="drop">
        <div class="font-12 m-r-10">上传文件:</div>
      ...
      <ul style="margin-top: 15px" class="imgList">
        <li v-for="(item, index) in fileList" v-if="fileList.length > 0" style="line-height: 25.6px; margin-top: 5px">
          ...
          <div class="liBox" @click="downLoad(item)">
            <div class="disStart">
              <img src="~@/assets/image/iconImg/pdf.png" class="imgBox" v-if="item.fileType == 'pdf'"/>
              <img src="~@/assets/image/iconImg/word.png" class="imgBox" v-if="item.fileType == 'doc'"/>
              <img src="~@/assets/image/iconImg/xls.png" class="imgBox" v-if="item.fileType == 'xls'"/>
              <img src="~@/assets/image/iconImg/jpg.png" class="imgBox" v-if="item.fileType == 'img'"/>
      ...
      <template slot="footer" class=" m-t-10">
        <div class="footerBtn"
             v-show="fileList.length > 0 && fileList[0].fileUrl && fileList[0].fileUrl !== 'undefined'">
...
export default {
  ...
  data() {
    return {
      ...
      fileList: [],
      url: {
        upimg: this.$config['domianURL'] + '/sys/common/upload',
      },
      loading: false,
      error: '',
      warn:'',
  ...
  methods: {
    ...
    async changeImg(e) {
      ...
      } else if (fileArray[fileArray.length - 1] == 'xls') {
        fileObj.fileType = 'xls'
      } else if (fileArray[fileArray.length - 1] == 'xlsx') {
        fileObj.fileType = 'xlsx'
      ...
      console.log("upimg===", this.url.upimg)
    ...
    handleOk() {
      if (this.fileList.length == 0) {
        this.$message.warning('请上传至少一个xls文件！')
        return
      }
      let params = {
      ...
      this.isWarn=true
      this.warn="请耐心等待，导入中..."
      getAction(this.importExcel, params).then(res => {
        this.loading = false
        this.isWarn=false
        if (res.success) {
