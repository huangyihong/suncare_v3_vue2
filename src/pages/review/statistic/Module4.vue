  ...
  export default {
    ...
    data() {
      this.url = {
        ...
        list: this.$suncare_v3  +this.$gp_database+ '/apiReviewStatisticNew/module4',
        exportXlsUrl: this.$suncare_v3  +this.$gp_database+ '/apiReviewStatisticNew/module4Export',
        ...
        if(this.dataSource && this.dataSource.length > 0){
          ...
          for(let {code, value} of this.classifyData){
            ...
            this.columns.push({
              ...
              customRender: (t, r, i) => {
                let val = r[code]
                return val?<div>{val}<span class="fr t-red">{(val * 100.0 / r.totalCount).toFixed(2) + '%'}</span></div>: 0
              },
            })
          }
        }
