<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="24">
          ...
          <a-col :md="6" :sm="8">
            <a-form-item label="不合规行为">
              <a-input placeholder="请输入(模糊查询)" v-model="queryParam.searchActionName"></a-input>
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="8">
    ...
    <div class="table-operator">
      ...
      <a-button @click="$refs.orderRef.edit('defSelect')" type="primary" icon="sort-ascending">调整默认字段顺序</a-button>
      <a-button @click="$refs.orderRef.edit('defSearch')" type="primary" icon="sort-ascending">调整默认搜索顺序</a-button>
    ...
    <div>
      ...
      <a-table
        ...
        <div slot="action" slot-scope="text, record">
          ...
          <a-divider type="vertical"/>
          <a @click="handleEditGroupby(record)">
            <a-icon type="loading"
                    v-if="record.groupFieldsLoading"></a-icon>
            汇总配置</a>
          <a-divider type="vertical"/>
    ...
    <!-- 表单区域 -->
    <taskActionFieldConfig-modal ref="modalForm" @ok="modalFormOk"></taskActionFieldConfig-modal>
    ...
    <!--调整默认字段和默认搜索条件顺序-->
    <task-action-col-order-modal ref="orderRef"></task-action-col-order-modal>
  </a-card>
</template>

<script>
  import TaskActionFieldConfigModal from './modules/TaskActionFieldConfigModal'
  ...
  import TaskActionColOrderModal from '@/pages/task/plugin/TaskActionColOrderModal'

  export default {
    name: 'TaskActionFieldConfigList',
    mixins: [JeecgListMixin],
    components: {
      TaskActionFieldConfigModal,
      TaskActionFieldConfigDetailModal,
      MDictSelectTag,
      GroupbyModal,
      AsyncLogModalButton,
      TaskActionColOrderModal
    },
    data() {
      return {
    ...
    methods: {
      ...
      handleEditGroupby(record) {
        this.$refs.groupbyModal.edit(record, groupFields => {
          console.log('groupFields', groupFields)
          return new Promise((resolve, reject) => {
            putAction(this.url.edit, { id: record.id, groupFields }).then((res) => {
              if (res.success) {
                record.groupFields = groupFields
                resolve()
              } else {
