  ...
  <a-card :bodyStyle="{ paddingBottom: '10px',paddingTop: '10px'}" :bordered="false">
    <div class="tabBox ">
      ...
      <div :class="[isActive == 2 ? 'activeBtn' : 'tabBtn']" @click="tabCard(2)" class="m-l-20">
        <div class="tabTitle"><a-icon type="pic-center" class="m-r-6" />审核任务</div>
      </div>
    ...
    <template v-if="isActive== 1">
      ...
      <a-button type="primary"  @click="handleImportExcel()" class="m-r-10">线索上传</a-button>
      <a-button type="primary"  @click="goBatchEdit()" class="m-r-10">批量修改</a-button>
      <a-button type="primary" @click="goBatchAudit('createByClue')" class="m-r-10">线索审核</a-button>
      ...
      <div>
        <a-table
          ...
          @change="handleTableChange">
            <span slot="auditState" slot-scope="text, record">
              <a-tag color="rgba(253, 194, 116, 1)" v-if="text==='init'">待审核</a-tag>
              <a-tag color="rgba(98, 222, 156, 1)" v-if="text==='finish'">已通过</a-tag>
              <a-tag color="rgba(255, 121, 121, 1)" v-if="text==='reject'">已驳回</a-tag>
              <a-tag color="rgba(255, 121, 121, 1)" v-if="text==='fail'">不通过</a-tag>
            </span>
          <span slot="action" slot-scope="text, record">
              ...
              <a-dropdown>
                <a-menu slot="overlay">
                  <a-menu-item key="1">
                    <a @click="handleEditFn(record)">线索修改</a>
                  </a-menu-item>
                  <a-menu-item key="2">
                  ...
                  <a-menu-item key="3"  v-if="record.auditState!='finish'">
                    <a-popconfirm title="确定审核通过当前项目线索信息吗?" @confirm="() => goAudit(record,'finish')">
                      <a >审核通过</a>
                    </a-popconfirm>
                  </a-menu-item>
                  <a-menu-item key="4"  v-if="record.auditState!='reject'">
                    <a-popconfirm title="确定驳回当前项目线索信息吗?" @confirm="() => goAudit(record,'reject')">
                      <a>驳回</a>
                    </a-popconfirm>
                  </a-menu-item>
                  <a-menu-item key="5">
      ...
      <!--导入-->
      <import-modal ref="importModal" @ok="()=>{expandedRowKeys = [];refreshData()}"></import-modal>
      ...
      <!--批量修改-->
      <batch-edit-modal ref="batchEditModal" @ok="refreshData"></batch-edit-modal>

      <!--推送其他环节-->
      <push-modal ref="pushModal" currStep="submit" @ok="refreshData"></push-modal>
    ...
    <template  v-if="isActive== 2">
      <project-task ref="projectTaskModal" processStep="task" :projectOrgId="$route.query.projectOrgId" @showTaskInfo="showTaskInfo">
        <div slot="createTaskBtn">
          <a-button type="primary"  @click="goBatchAudit('createByUpload')" class="m-r-10">发起审核</a-button>
        </div>
      </project-task>
    </template>

    <template  v-if="isActive== 3">
      <clue-file ref="clueFileModal" :projectOrgId="$route.query.projectOrgId" processStep="task"></clue-file>
    </template>

    <!-- 线索审核 -->
    <project-task-modal ref="taskModal" @ok="refreshTaskList" processStep="task"></project-task-modal>
  </a-card>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        ...
        statClueInSubmit:"/fj/clue/statClueInSubmit",
  ...
  methods: {
    ...
    getStatStepClueVo(){
      ...
      getAction(this.url.statClueInSubmit, { projectOrgId }).then(res => {
        if (res.success) {
    ...
    handleEditFn(record){
      if(record.auditState=='finish'){
        this.$message.warning('线索已审核通过，不允许修改！');
        return
      }
      if(record.currStep!='submit'){
        this.$message.warning('线索已推送到其他环节，不允许修改！');
        return
      }
    ...
    goBatchAudit(fnType) {
      if(fnType=='createByClue'){
        if(this.selectedRowKeys.length===0){
          this.$message.error("请您至少选择一条记录进行线索审核操作")
          return
        }
        //判断状态是否为驳回
        let flag = true
        this.selectionRows.forEach(item=>{
          if(item.auditState=='reject'){
            this.$message.error("包含线索状态为‘已驳回’的数据,无法进行线索审核,请重新选择数据");
            flag = false
            return;
          }
        })
    ...
    handleImportExcel() {
      let param = {
        ...
        param:{projectOrgId:this.$route.query.projectOrgId},
      }
      this.$refs.importModal.loadData(param);
    },
    goBatchEdit(){
      if(this.selectedRowKeys.length!=1){
        this.$message.error("请选择一条记录进行线索修改操作")
        return
      }
      let item = this.selectionRows[0]
      if(item.auditState=='finish'){
        this.$message.error("审批状态为‘已通过’,无法进行修改");
        flag = false
        return;
      }
      this.$refs.batchEditModal.loadData({fnType:'cover',clueId:this.selectedRowKeys[0],projectOrgId:this.$route.query.projectOrgId,
        title:'批量修改',
        uploadFileName:'线索提交及统计汇总表.xlsx',
        templateUrl:'/fj/clue/download/clue',
        actionUrl:'/fj/clue/',
