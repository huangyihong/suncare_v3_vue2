  ...
  <a-card :bodyStyle="{ paddingBottom: '10px',paddingTop: '10px'}" :bordered="false">
    <div class="tabBox ">
      ...
      <div :class="[isActive == 2 ? 'activeBtn' : 'tabBtn']" @click="tabCard(2)" class="m-l-20">
        <div class="tabTitle"><a-icon type="pic-center" class="m-r-6" />核减进度</div>
      </div>
    </div>
    ...
    <template v-if="isActive== 1">
      ...
      <a-button type="primary" @click="handleExportExcel()" class="m-r-10" >导出</a-button>
      <a-button  @click="goBatchAudit('createByClue')" class="m-r-10" style="float: right">线索申诉</a-button>
    ...
    <template  v-if="isActive== 2">
      <project-task ref="projectTaskModal" processStep="taskCutOrg" :projectOrgId="$route.query.projectOrgId" @showTaskInfo="showTaskInfo">
        <div slot="createTaskBtn">
          <a-button type="primary"  @click="goBatchAudit('createByUpload')" class="m-r-10">线索申诉</a-button>
        </div>
      </project-task>
    </template>

    <!-- 反馈信息 -->
    <project-task-modal ref="taskModal" @ok="refreshTaskList" processStep="taskCutOrg"></project-task-modal>
  ...
  data() {
    return {
      ...
      url: {
        ...
        exportXlsUrl: "/fj/clue/exportByOrg",
    ...
    goBatchAudit(fnType) {
      if(fnType=='createByClue'){
        if(this.selectedRowKeys.length===0){
          this.$message.error("请您至少选择一条记录进行线索申诉操作")
          return
        }
