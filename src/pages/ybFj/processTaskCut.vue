  ...
  <a-card :bodyStyle="{ paddingBottom: '10px',paddingTop: '10px'}" :bordered="false">
    <div class="tabBox ">
      ...
      <div :class="[isActive == 2 ? 'activeBtn' : 'tabBtn']" @click="tabCard(2)" class="m-l-20">
        <div class="tabTitle"><a-icon type="pic-center" class="m-r-6" />核减审批</div>
      </div>
      <div :class="[isActive == 3 ? 'activeBtn' : 'tabBtn']" @click="tabCard(3)" class="m-l-20">
        <div class="tabTitle"><a-icon type="folder-add" class="m-r-6" />核减材料</div>
      </div>
    </div>
    ...
    <template v-if="isActive== 1">
     ...
     <!-- <a-button type="primary" @click="goBatchAudit('createByClue')" class="m-r-10">代发核减</a-button>-->
      <a-button type="primary" @click="pushOrg" class="m-r-10">同步至医院</a-button>
      ...
      <div>
        <a-table
          ...
          @change="handleTableChange">
            <span slot="cutAuditState" slot-scope="text, record">
              <a-tag color="rgba(253, 194, 116, 1)" v-if="text==='init'">待核减</a-tag>
              <a-tag color="rgba(98, 222, 156, 1)" v-if="text==='finish'">已核减</a-tag>
            </span>
          <span slot="action" slot-scope="text, record">
              ...
              <a-dropdown>
                <a-menu slot="overlay">
                  ...
                  <a-menu-item key="3"  v-if="record.cutAuditState!='finish1'">
                    <a-popconfirm title="确定核减全部当前项目线索信息吗?" @confirm="() => cutAll(record)">
                      <a >核减全部</a>
                    </a-popconfirm>

                  </a-menu-item>
                  <a-menu-item key="4"  v-if="record.cutAuditState!='finish'">
                    <a @click="cutPartShow(record)">核减部分</a>
                  </a-menu-item>
                </a-menu>
      ...
      <!--推送其他环节-->
      <push-modal ref="pushModal" currStep="cut" @ok="refreshData"></push-modal>
    ...
    <template  v-if="isActive== 2">
      <project-task ref="projectTaskModal" processStep="taskCut" :projectOrgId="$route.query.projectOrgId" @showTaskInfo="showTaskInfo">
        <!--<div slot="createTaskBtn">
          <a-button type="primary"  @click="goBatchAudit('createByUpload')" class="m-r-10">发起核减</a-button>
        </div>-->
      </project-task>
    </template>

    <template  v-if="isActive== 3">
      <clue-file ref="clueFileModal" :projectOrgId="$route.query.projectOrgId" processStep="taskCut"></clue-file>
    </template>

    <!-- 反馈信息 -->
    <project-task-modal ref="taskModal" @ok="refreshTaskList" processStep="taskCut"></project-task-modal>

    <!-- 核减部分 -->
    <m-modal centered
             title="核减部分"
             :width="500"
             :visible="cutPartVisible"
             @ok="handleCutPartSave"
             @cancel="handleCutPartClose"
             cancelText="关闭">
      <a-form :form="form">
        <a-form-item  :labelCol="labelCol"
                      :wrapperCol="wrapperCol" label="核减数量">
          <a-input-number placeholder="核减数量" :step="0.01"  style="width: 100%"
                          v-decorator="['cutAmount',validatorRules.cutAmount]"/>
        </a-form-item>
        <a-form-item  :labelCol="labelCol"
                      :wrapperCol="wrapperCol" label="核减医保基金金额">
          <a-input-number placeholder="核减医保基金金额" :step="0.01"  style="width: 100%"
                          v-decorator="['cutFundFee',validatorRules.cutFundFee]"/>
        </a-form-item>
      </a-form>
...
<script>
import {httpAction, getAction, putAction,postAction, downFile} from '@/api/manage'
...
export default {
  ...
  data() {
    return {
      ...
      validatorRules: {
        cutFundFee:{rules:[{ required: true, message: '这是必填项!' },{ pattern: /^(0|[1-9])+(\.[0-9]{1,2}){0,1}$/,trigger: "blur", message: "只能输入整数或小数,小数位数不超过2"}]},
        cutAmount: { rules: [{ required: true, message: '这是必填项!' },{ pattern: /^[1-9]+\d*$/,trigger: "blur", message: "只能输入整数"}] },
      },
      columns: [
        {
          title: '问题类别',
          ...
          title: '审核状态',
          align: "left",
          dataIndex: 'cutAuditState',
          fixed: 'right',
          width: '85px',
          scopedSlots: {customRender: 'cutAuditState'}
        },{
          title: '审核人',
          align: "left",
          dataIndex: 'cutAuditUserName',
          fixed: 'right',
          width: '80px'
        },{
          title: '审核时间',
          align: "left",
          dataIndex: 'cutAuditTime',
          fixed: 'right',
          width: '160px'
        },{
          title: '操作',
  ...
  methods: {
    ...
    goAudit(record,auditState) {
      ...
      postAction_sv3(this.url.audit,{auditState,clueId:record.clueId,auditOpinion:''}).then((res)=>{
        if(res.success){
    ...
    goBatchAudit(fnType) {
      if(fnType=='createByClue'){
        if(this.selectedRowKeys.length===0){
          this.$message.error("请您至少选择一条记录进行代发核减操作")
          return
        }
    ...
    downloadZip(clueId){
      ...
      this.downloadFileAction(this.url.downloadZip,{clueId},fileName)
    },
    refreshTaskList(){
      if(this.isActive=='2'){
    ...
    cutAll(record){
      this.confirmLoading = true;
      postAction(this.url.cutSave,{clueId:record.clueId,cutAmount:record.caseAmount,cutFundFee:record.caseFundFee}).then((res)=>{
        if(res.success){
          this.$message.success(res.message);
          this.refreshData()
        }else{
    ...
    cutPartShow(record){
      ...
      this.cutPartClueId = record.clueId
      this.cutPartVisible = true
    },
    handleCutPartSave(){
      this.form.validateFields((err, values) => {
        if (!err) {
          console.log(values)
          let formData = Object.assign({clueId:this.cutPartClueId}, values);
          const that = this
          this.$confirm({
            title: '确认提交',
            content: '确定核减部分当前项目线索信息吗?',
            onOk: function() {
              that.cutAll(formData)
              that.cutPartVisible = false
            }
          })
        }
      })
    ...
    handleCutPartClose(){
      this.cutPartVisible = false
    },
    //同步医院
    pushOrg(){
      if(this.selectedRowKeys.length===0){
        this.$message.error("请您至少选择一条记录进行同步医院操作")
        return
      }
      let clueIds =  this.selectedRowKeys
      const that = this
      this.$confirm({
        title: '确认同步医院',
        content: '确定同步当前项目线索信息到医院端吗?',
        onOk: function() {
          that.pushOrgAction(clueIds.join(","))
        }
      })

    },
    pushOrgAction(clueIds){
      this.confirmLoading = true;
      postAction_sv3(this.url.pushOrg,{clueIds}).then((res)=>{
        if(res.success){
          this.$message.success(res.message);
        }else{
          this.$message.warning(res.message);
