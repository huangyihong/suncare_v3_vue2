<template>
  <div>
    ...
    >
      <select-component ref="selectRef" :selected="selectData"
                        :selectType="multi?'checkbox':'radio'"></select-component>
    </m-modal>
  </div>
</template>
...
import SelectComponent from '@/pages/selectMulti/MedicalDiseaseDiag'
import selectInputMixin from './mixin'

export default {
  name: 'MedicalDiseaseDiag',
  ...
  methods: {
    loadData(codes) {
      this.loading = true
      getAction(`/config/medicalDiseaseDiag/queryByCodes`, { codes }).then((res) => {
        if (res.success) {
          let data = res.result
