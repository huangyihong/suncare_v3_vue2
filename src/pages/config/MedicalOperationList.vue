<template>
  <a-card :bordered="false">
    <a-layout>
      <a-layout-sider collapsedWidth="0" collapsible :trigger="null" v-model="sideCollapsed"  width="225" :style="{ overflow: 'auto', height: '100vh',  background: 'white' }" >
        <div>
          <div class="icons-list" >
            <a-icon type="edit"  theme="twoTone"  @click="openOtherDictTab('STD_ICD_surgery')"  style="float:right;margin-right: 20px;fontSize:18px;"/>
          </div>
      ...
      <a-layout-content style="padding: 0 20px 20px 20px;background: #fff;border: 5px; border-left-style: outset;">
            ...
            <a-row :gutter="24">
              <a-col :md="8" :sm="12">
                <a-form-item label="手术及操作编码">
                  <j-input placeholder="输入手术及操作编码模糊查询(,代表与|代表或)" type="more" v-model="queryParam.code"/>
                </a-form-item>
              </a-col>
              <a-col :md="8" :sm="12">
                <a-form-item label="手术及操作名称">
                  <j-input placeholder="输入手术及操作名称模糊查询(,代表与|代表或)" type="more" v-model="queryParam.name"/>
                </a-form-item>
              </a-col>
        ...
        <div class="table-operator">
          ...
          <a-button type="primary" v-has="'basicDataManage:export'" icon="download" @click="handleExportExcel('手术信息')">导出</a-button>
        ...
        <span slot="name" slot-scope="text, record">
          <a @click="handleDetail(record)"><j-ellipsis :value="text" :length="15"/></a>
        </span>
    ...
    <!-- 表单区域 -->
    <medical-operation-modal ref="modalForm" @ok="myModalFormOk"></medical-operation-modal>
    ...
    <m-modal centered
             ...
             cancelText="关闭">
      <select-medical-operation ref="selectRef" :selected="selectData" selectType="radio"
                                   :fixedParam="{ state: '1' }"></select-medical-operation>
    </m-modal>
...
<script>
  ...
  import MedicalOperationModal from './modules/MedicalOperationModal.vue'
  import OtherDictTree from './modules/OtherDictTree.vue'

  const SelectMedicalOperation = () => import("@/pages/selectMulti/MedicalOperation")

  export default {
    name: "MedicalOperationList",
    mixins:[JeecgListMixin,MedicalAuditLogListMixin,CommonReplaceDataMixin],
    components: {
      MedicalOperationModal,
      JDictSelectTag,
      OtherDictTree,
      SelectMedicalOperation,
    },

    data () {
      return {
        description: '手术信息管理页面',
        // 表头
        columns: [
          {
            title: '手术及操作编码',
        ...
        url: {
          list: "/config/medicalOperation/manageList",
          delete: "/config/medicalOperation/delete",
          deleteBatch: "/config/medicalOperation/deleteBatch",
          exportXlsUrl: "/config/medicalOperation/exportXls",
          importExcelUrl: "/config/medicalOperation/importExcel",
          getDataCount: "/config/medicalOperation/getDataCount",
          saveClean:"/config/medicalOperation/saveCleanMedicalOperation",
          saveAuditAll:"/config/medicalOperation/saveAuditAllMedicalAuditLog",
          saveUndoAllMedicalAuditLog: "/config/medicalOperation/saveUndoAllMedicalAuditLog",
          exportExcelByThread: "/config/medicalOperation/exportExcelByThread",
          exportExcel: "/config/medicalOperation/exportExcel",
          replaceData: "/config/medicalOperation/replaceData",
        },
        keyFiled:'id',
