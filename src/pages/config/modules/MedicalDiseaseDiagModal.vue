            ...
            <a-col :md="12" :sm="24">
              <a-form-item
                ...
                <j-tree-select
                  ...
                  pid-field="parent_code" condition='{"dict_ename":"STD_ICDClassify","state":"1"}'
                  placeholder="请选择父级分类名称" @change="selectTypeCode" v-if="!readonly">
                </j-tree-select>

                <a-input placeholder="请选择父级分类名称" v-decorator="['typeName', validatorRules.typeCode]" readOnly="true" v-if="readonly"/>
              </a-form-item>
            </a-col>
            <a-col :md="12" :sm="24">
              <a-form-item
                ...
                :wrapperCol="wrapperCol"
                label="父级分类编码">
                <a-input placeholder="请输入父级分类编码" v-decorator="['typeCodeText',{}]" readOnly="true"/>
              </a-form-item>
            </a-col>
            <a-col :md="12" :sm="24">
              <a-form-item
                ...
                :wrapperCol="wrapperCol"
                label="疾病编码">
                <a-input placeholder="请输入疾病编码" v-decorator="['code', validatorRules.code]" :readOnly="!!model.id"/>
              </a-form-item>
            </a-col>
            <a-col :md="12" :sm="24">
              <a-form-item
                ...
                :wrapperCol="wrapperCol"
                label="疾病名称">
                <a-input placeholder="请输入疾病名称" v-decorator="['name', validatorRules.name]" :readOnly="readonly"/>
              </a-form-item>
            </a-col>
            <a-col :md="24" :sm="24">
              <a-form-item
  ...
  export default {
    ...
    data () {
      return {
        ...
        validatorRules:{
          code:{rules: [{ required: true, message: '请输入疾病编码!' }]},
          name:{rules: [{ required: true, message: '请输入疾病名称!' }]},
          typeCode:{rules: [{ required: true, message: '请选择父级分类!' }]},
        },
        url: {
          add: "/config/medicalDiseaseDiag/add",
          edit: "/config/medicalDiseaseDiag/edit",
          isExistName: "/config/medicalDiseaseDiag/isExistName",
          del: "/config/medicalDiseaseDiag/delMedicalDiseaseDiag",
          delAll: "/config/medicalDiseaseDiag/delAllMedicalDiseaseDiag",
        },
        typeCodeText:'',
      }
    },
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            getAction(this.url.isExistName,param,'get').then((data) => {
              if (data.success) {
                if(data.result){
                  that.$message.warning('该疾病编码字段值已存在，请重新输入');
                }else{
                  that.save(values);
                }
              }else{
