          ...
          <template v-if="model.fntype!=='batchDelete'">
            <a-col :md="12" :sm="24">
              <a-form-item
                ...
                <j-tree-select
                  ...
                  pid-field="parent_code" condition='{"dict_ename":"STD_ICD_surgery","state":"1"}'
            ...
            <a-col :md="12" :sm="24">
              <a-form-item
                ...
                :wrapperCol="wrapperCol"
                label="手术及操作编码">
                <a-input placeholder="请输入手术及操作编码" v-decorator="['code', validatorRules.code]" :readOnly="!!model.id"/>
              </a-form-item>
            </a-col>
            <a-col :md="12" :sm="24">
              <a-form-item
                ...
                :wrapperCol="wrapperCol"
                label="手术及操作名称">
                <a-input placeholder="请输入手术及操作名称" v-decorator="['name', validatorRules.name]" :readOnly="readonly"/>
              </a-form-item>
            </a-col>
            <a-col :md="12" :sm="24">
              <a-form-item
                ...
                :wrapperCol="wrapperCol"
                label="手术级别">
                <other-dict-select-tag  placeholder="请选择手术级别" v-model="operationLevelCodeSelect" dictEname="SurgeryLevel" labelInValue="true" :readOnly="readonly"/>
              </a-form-item>
            </a-col>
            <a-col :md="12" :sm="24">
              <a-form-item
                ...
                :wrapperCol="wrapperCol"
                label="手术无菌程度">
                <other-dict-select-tag  placeholder="请选择手术无菌程度" v-model="bacterialLevelSelect" dictEname="BacterialLevel" labelInValue="true" :readOnly="readonly"/>
              </a-form-item>
            </a-col>
            <a-col :md="24" :sm="24">
              <a-form-item
  ...
  export default {
    ...
    data () {
      return {
        ...
        validatorRules:{
          code:{rules: [{ required: true, message: '请输入手术及操作编码!' }]},
          name:{rules: [{ required: true, message: '请输入手术及操作名称!' }]},
          typeCode:{rules: [{ required: true, message: '请选择上级分类!' }]},
        },
        url: {
          add: "/config/medicalOperation/add",
          edit: "/config/medicalOperation/edit",
          isExistName: "/config/medicalOperation/isExistName",
          del: "/config/medicalOperation/delMedicalOperation",
          delAll: "/config/medicalOperation/delAllMedicalOperation",
          getTypeList: "/config/medicalOperation/getTypeList",
        },
        typeCodeText:'',
        operationLevelCodeSelect:{},
        bacterialLevelSelect:{},
      }
    },
    beforeMount(){
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'code','name','typeName','remark','state','auditResult','actionType','createStaff','createStaffName','createReason','updateStaff','updateStaffName','updateReason','deleteStaff','deleteStaffName','deleteReason','actionStaff','actionStaffName','auditStaff','auditStaffName','auditOpinion'))
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            getAction(this.url.isExistName,param,'get').then((data) => {
              if (data.success) {
                if(data.result){
                  that.$message.warning('该手术及操作编码字段值已存在，请重新输入');
                }else{
                  that.save(values);
                }
              }else{
      ...
      setSelectValue(){
        this.operationLevelCodeSelect ={key:this.model.operationLevelCode?this.model.operationLevelCode:''};
        this.bacterialLevelSelect = {key:this.model.bacterialLevel?this.model.bacterialLevel:''};
      },
      getSelectText(){
        if(this.operationLevelCodeSelect){
          this.model.operationLevelCode = this.operationLevelCodeSelect.key
          this.model.operationLevel = this.operationLevelCodeSelect.label
        }
        if(this.bacterialLevelSelect){
          this.model.bacterialLevel = this.bacterialLevelSelect.key
          this.model.bacterialLevelName = this.bacterialLevelSelect.label
        }
      },
