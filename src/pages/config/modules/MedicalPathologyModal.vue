          ...
          <template v-if="model.fntype!=='batchDelete'">
            <a-col :md="12" :sm="24">
              <a-form-item
                ...
                label="YX形态学编码">
                <a-input placeholder="请输入YX形态学编码" v-decorator="['code', validatorRules.code]" :readOnly="!!model.id"/>
              </a-form-item>
            </a-col>
            <a-col :md="12" :sm="24">
              <a-form-item
                ...
                :wrapperCol="wrapperCol"
                label="YX形态学名称">
                <a-input placeholder="请输入YX形态学名称" v-decorator="['name', validatorRules.name]" :readOnly="readonly"/>
              </a-form-item>
            </a-col>
            <a-col :md="12" :sm="24">
              <a-form-item
                ...
                <j-tree-select
                  ...
                  pid-field="parent_code" condition='{"dict_ename":"pathologyclass","state":"1"}'
                  placeholder="请选择父级分类名称" @change="selectParentCode" v-if="!readonly">
                </j-tree-select>

                <a-input placeholder="请选择父级分类名称" v-decorator="['parentName', validatorRules.parentCode]" readOnly="true" v-if="readonly"/>
              </a-form-item>
            </a-col>
            <a-col :md="12" :sm="24">
              <a-form-item
                ...
                :wrapperCol="wrapperCol"
                label="父级分类编码">
                <a-input placeholder="请输入父级分类编码" v-decorator="['parentCodeText',{}]" readOnly="true"/>
              </a-form-item>
            </a-col>
            <a-col :md="24" :sm="24">
              <a-form-item
                ...
                <a-textarea placeholder="请输入描述" v-decorator="['descript', {}]" :rows="3" :readOnly="readonly"/>
              </a-form-item>
            </a-col>
  ...
  export default {
    ...
    data () {
      return {
        ...
        validatorRules:{
          code:{rules: [{ required: true, message: '请输入YX形态学编码!' }]},
          name:{rules: [{ required: true, message: '请输入YX形态学名称!' }]},
          parentCode:{rules: [{ required: true, message: '请选择父级分类!' }]},
        },
        url: {
          add: "/config/medicalPathology/add",
          edit: "/config/medicalPathology/edit",
          isExistName: "/config/medicalPathology/isExistName",
          del: "/config/medicalPathology/delMedicalPathology",
          delAll: "/config/medicalPathology/delAllMedicalPathology",
        },
        parentCodeText:'',
      }
    },
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'code','name','parentName','descript','state','auditResult','actionType','createStaff','createStaffName','createReason','updateStaff','updateStaffName','updateReason','deleteStaff','deleteStaffName','deleteReason','actionStaff','actionStaffName','auditStaff','auditStaffName','auditOpinion'))
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            getAction(this.url.isExistName,param,'get').then((data) => {
              if (data.success) {
                if(data.result){
                  that.$message.warning('该YX形态学编码已存在，请修改为其他YX形态学编码');
                }else{
                  that.save(values);
                }
              }else{
