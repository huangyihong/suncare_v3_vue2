<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="24">
          ...
          <a-col :md="6" :sm="8">
            <a-form-item label="项目地名称">
              <j-input placeholder="请输入项目地名称" v-model="queryParam.project"></j-input>
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="8">
            <a-form-item label="物价级别编码">
              <a-input placeholder="请输入物价级别编码" v-model="queryParam.hosplevel"></a-input>
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="8">
            <a-form-item label="物价级别名称">
              <j-input placeholder="请输入物价级别名称" v-model="queryParam.hoslevelName"></j-input>
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="8">
    ...
    <!-- 表单区域 -->
    <stdHoslevelFundpaypropModal ref="modalForm" @ok="modalFormOk"></stdHoslevelFundpaypropModal>
...
<script>
import StdHoslevelFundpaypropModal from './modules/StdHoslevelFundpaypropModal'
...
export default {
  name: "StdHoslevelFundpaypropList",
  mixins: [JeecgListMixin],
  components: {
    StdHoslevelFundpaypropModal,
    ImportModal,
    moment
  },
  data() {
    return {
      description: '各地不同物价级别报销比例管理页面',
      // 表头
      columns: [
        {
          title: '项目地名称',
        ...
        },
        {
          ...
          width:150,
          align:"left",
          dataIndex: 'hosplevelName'
        },
        {
          title: '报销比例(%)',
          width:150,
          align:"left",
          dataIndex: 'fundpayprop'
        },
        {
          title: '适用开始时间',
          width:150,
      ...
      url: {
        list: "/config/stdHoslevelFundpayprop/list",
        delete: "/config/stdHoslevelFundpayprop/delete",
        deleteBatch: "/config/stdHoslevelFundpayprop/deleteBatch",
        exportXlsUrl: "config/stdHoslevelFundpayprop/exportXls",
        importExcelUrl: "config/stdHoslevelFundpayprop/importExcel",
        exportExcel: "/config/stdHoslevelFundpayprop/exportExcel",
      },
  ...
  methods: {
    ...
    handleExportExcel() {
      ...
      this.handleExportFile('各地不同物价级别报销比例' + moment().format('YYYYMMDDHHmmss')+'.xlsx')
    },
    //导入
    handleImportExcel() {
      let param = {
        title: '各地不同物价级别报销比例导入',
        importUrl: this.url.importExcelUrl,
        actionType: 'add',
        uploadFileName: 'STD_HOSLEVEL_FUNDPAYPROP.xlsx'
      }
