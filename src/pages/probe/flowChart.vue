    ...
    <div>
        ...
        <div :style="{height:  chartHeight}">
            ...
            <div class="chart" style="position: relative">
                ...
                <ul :id="menuId" class="ctxmenu">
                    ...
                    <li id="saveTmpl" class="menu-item" @click="saveToTmpl">保存为模板</li>
        ...
        <a-tabs v-model="tabKey" @change="tabChange">
            <a-tab-pane tab="节点详情" key="1" :disabled="!selectKey" style=";overflow-x: auto" :forceRender="true">
                <rule-content ref="ruleRef" :actionBtnType="actionBtnType" @action="()=> {$emit('action')}"
                              :value="selectName"
                              @input="updateSelectNodeText"
                              :readonly="readonly" @test="test"
                              @typeChange="updateSelectNodeType"
                              @linkToVirtual="openNodeSelect(selectKey)"
                              @cancelToVirtual="cancelToVirtual"
                              style="min-width: 900px"
                              :nodeKey="selectKey" :nodeCategory="selectCategory"
                              :nodeType="selectType"
                              :nodeVCode="selectVCode"
                              :uniqueVal="uniqueVal"
                              @tableNameChange="val => tableName = val"
                              v-show="selectKey"></rule-content>
            </a-tab-pane>
            ...
            <a-tab-pane tab="流程结果" key="3" :forceRender="true">
                <case-result :nodes="nodeDataArray" :keyPre="keyPre" :caseData="caseTestParams"
                             :uniqueVal="uniqueVal"
                             @getParams="keys=>caseTestParams = getTestParam(keys)" @getNodes="setNodeArray"
                             :key="diff"></case-result>
            </a-tab-pane>
        </a-tabs>
        ...
        <m-modal centered
                 ...
                 @cancel="handleSelectNodeCancel"
                 :destroyOnClose="true"
                 cancelText="关闭">
            <select-medical-flow-templ ref="selectNodeRef" :nodeTypeSelect="!selectNodeCode" :selectType="selectNodeCode?'radio':'checkbox'"></select-medical-flow-templ>
        </m-modal>
        <medical-flow-templ-modal ref="tmplModal"></medical-flow-templ-modal>
    </div>
</template>

<script>
  import RuleContent from '@/pages/probe/case/caseRule';
  ...
  import { queryFlowTemplRulesByIds } from '@/api-sv3/api-flow'
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        selectNodeCode: undefined,
        menuFocusNode: undefined,
      }
    },
    beforeMount() {
      //  保证不冲突
    ...
    methods: {
      initFlowChart() {
        ...
        this.flowChartObj = new FlowChart({
          ...
          nodeDelete: (nodes) => {
            ...
            let keys = nodes.filter(r => r.data && r.data.category === 'rectangle' || r.data.category === 'conditional').map(r => r.key)
            if(keys.length > 0){
          ...
          selectActive: (data) => {
            ...
            if (category === 'rectangle' || category === 'conditional') { // 节点
              this.selectVCode = data.vCode
              this.selectKey = data.key
              if (this.tabKey !== '1') {
                this.tabKey = '1'
              }
            } else {
              this.selectVCode = undefined
            }

          },
          menuFocus: (node) => node && (this.menuFocusNode = node.data)
        })
      },
      pasteTextOK(text) {
      ...
      updateNodeCount(key,  val){
        this.flowChartObj.updateProp(key, 'count',  val)
      },
      clearCount(){
        this.flowChartObj.updateAllProp('count', null)
      },
      updateSelectNodeType(val) {
        this.selectType = val
        this.flowChartObj.updateType(this.selectKey, val)
      },
      openNodeSelect(nodeCode, linkVirtual){
        this.selectNodeVisible = true
        this.selectNodeCode = nodeCode
        this.selectNodelinkVirtual = linkVirtual
      },
      handleSelectNodeCancel(){
        this.selectNodeVisible = false
      },
      handleSelectNodeOk(){
        this.selectNodeVisible = false
        let list = this.$refs.selectNodeRef.getSelect()
        if(list.length === 0){
          return
        }

        const idKeyMap = new Map()
        console.log('this.selectNodeCode',this.selectNodeCode)
        // 添加节点
        if(!this.selectNodeCode){
          let index = 0
          const nodes = []
          for(let item of list){
            let {id,name,type = 'rect',code, tableName} = item
            let prop = {}
            if(tableName !== 'DWB'){
              type += '_dws'
            }
            // 虚拟节点
            if(this.selectNodelinkVirtual){
              name = `(${code})${name}`
              type = type + '_v'
              prop = {param: id, vCode: code}
            }

            console.log('addNode', item)
            let key = this.addNode(type, name, `${index++ * 300 + -200} -100 `, prop)
            nodes.push({key,type})
            idKeyMap.set(id,key)
          }
          // 串联节点
          for(let i = 1, len = nodes.length; i < len; i++){
            this.addLink(nodes[i - 1],nodes[i])
          }
        } else {
          let {id,name,code,tableName} = list[0]
          // 链接到当前选中的节点
          this.linkToVirtual(id, name, code, tableName !== 'DWB')
          let key = this.selectNodeCode
          idKeyMap.set(id,key)
          this.removeRules([key])
        }

        this.setOutLoading(true)
        queryFlowTemplRulesByIds({ids: Array.from(idKeyMap.keys()).join(",")}).then(res => {
          if (res.success) {
            let data = res.result
            data.forEach(item => {
              item.nodeCode = idKeyMap.get(item.nodeId);
              item.nodeId = item.ruleId = null
            })
            this.addRules(data)
            if(this.selectNodeCode === this.selectKey){
              this.refreshRules()
            }
          } else {
          ...
          this.setOutLoading(false)
        })
      },
      linkToVirtual(id, name, code, isDws){
        // 清空原后缀
        this.selectType = this.selectType.replace(/_dws|_v/g, '')
        if(isDws){
          this.selectType = this.selectType + '_dws'
        }
        this.selectType = this.selectType + '_v'
        this.selectVCode = code
        this.flowChartObj.updateNodeData(this.selectKey, {type: this.selectType ,param: id, vCode: this.selectVCode})
        this.updateNodeText(this.selectKey, `(${code})${name}`)
        /*if(this.selectName.startsWith('计算(') || this.selectName.startsWith('条件(')){
          console.log('updateNodeText',this.selectName,name)
          this.updateNodeText(this.selectKey, name)
        }*/
      },
      cancelToVirtual(){
        this.selectType = this.selectType.replace(/_dws|_v/g, '')
        ...
        this.selectVCode = undefined
        this.flowChartObj.updateNodeData(this.selectKey, {type: this.selectType ,param: undefined, vCode: this.selectVCode})
        this.removeRules([this.selectKey])
        this.refreshRules()
        // 还原到DWB表
        this.$refs.ruleRef.restoreTable(this.selectKey)
      ...
      getData() {
        ...
        flowJson.nodeDataArray.forEach(item => {delete item.count})
        let keys = Array.from(flowJson.nodeDataArray, (node) => !node.type.endsWith('_v') && node.key.toString())
        return {
      ...
      refreshRules(){
        this.$refs.ruleRef.nodeChange(this.selectKey)
      },
      getSubmitKeys() {
      ...
      addNode(type, text, loc, prop) {
        return this.flowChartObj.addNode(type, text, loc, prop)
      },
      addLink(from, to) {
      ...
      // 保存为模板
      saveToTmpl(e){
        let key = this.menuFocusNode.key
        let rules = this.$refs.ruleRef.getRules([key])
        this.$refs.tmplModal.edit({nodeName: this.menuFocusNode.text}, key, rules)
      },
      showSetting() {
