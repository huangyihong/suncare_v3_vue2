<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="download" @click="handleExportExcel(`诊疗合规规则`)">导出</a-button>
    ...
    <trail-drug-modal ref="trailModal" ruleType="4" @ok="modalFormOk"></trail-drug-modal>

    <!-- 表单区域 -->
    <medical-treat-rule-modal ref="modalForm" @ok="modalFormOk"></medical-treat-rule-modal>
...
<script>
  import MedicalTreatRuleModal from './modules/MedicalTreatRuleModal'
  ...
  import {treatLimitScope} from "@/pages/case/DrugRuleCommon"
...
//            },
        ...
        url: {
          ...
          importExcelUrl: "/medical/medicalDrugRule/importExcel?ruleType=4",
    ...
    methods: {
      filterDictText,
      limitLoaded(data){
        let limitSet = new Set(treatLimitScope.map(item => item.code))
      ...
      handleImportExcel() {
        let param = {
          ...
          actionType: 'add',
          uploadFileName: 'MEDICAL_TREAT_RULE.xls'
        }
        this.$refs.importModel.loadData(param);
      },
      gotoDrugView(ruleId) {
        this.$router.push({ path: '/review/reviewSystemCommonView', query: { ruleId, ruleType: 'TREAT' } })
      }
    }
  }
</script>
