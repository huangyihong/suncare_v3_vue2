<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="download" @click="handleExportExcel(`收费合规配置`)" v-if="hasExport">导出</a-button>
      <a-button type="primary" icon="download" @click="handleExportFile(`收费合规失效明细${moment().format('YYYYMMDDHHmmss')}.xlsx`,undefined, url.exportInvalidXls)" v-if="hasExport">导出失效明细</a-button>
    ...
    <!-- 表单区域 -->
    <medical-charge-rule2-modal ref="modalForm" @ok="modalFormOk"></medical-charge-rule2-modal>
...
<script>
  import MedicalChargeRule2Modal from './modules/MedicalChargeRule2Modal'
    ...
    data() {
      return {
        ...
        url: {
          ...
          importExcelUrl: "/medical/medicalRuleConfig/importExcel?ruleType=CHARGE",
        ...
        fixedParam: { ruleType: 'CHARGE' },
        runStatusDict: [],
        timer: undefined,
        ruleLimitMap: {},
        ruleLimitDictCode: 'RULE_LIMIT_CHARGE'
      }
    },
    beforeMount() {
    ...
    methods: {
      ...
      handleImportExcel() {
        let param = {
          ...
          actionType: 'add',
          uploadFileName: 'MEDICAL_CHARGE_RULE2.xlsx'
        }
        this.$refs.importModel.loadData(param);
      },
      gotoDrugView(ruleId) {
        this.$router.push({ path: '/review/reviewSystemCommonView', query: { ruleId, ruleType: 'CHARGE2' } })
      }
    }
  }
</script>
