  ...
  export default {
    ...
    methods: {
      loadData(record) {
        ...
        let {diseaseGroups, treatGroups, testResults} = record
        // 数组拼接字符串
        ...
        this.$nextTick(()=>{
          this.form.setFieldsValue({ ...pick(record,'groupName')
            , diseaseGroups: diseaseGroupsVal, treatGroups: treatGroupsVal, testResults: testResultsVal})
        })
      },
      close() {
        this.form.validateFieldsAndScroll((err, values) => {
          try {
            if (!err) {
              ...
              let {diseaseGroups, treatGroups, testResults, groupName} = values
              let obj = {groupName}
