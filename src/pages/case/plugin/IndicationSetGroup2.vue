<template>
    <div>
            ...
            <a-card hoverable style="margin-top: 10px" v-for="(item,index) in ruleGroup" :key="index">
                <a-card-meta>
                    ...
                    <template slot="description">
                        ...
                        <div style="line-height: 30px">
                            <template v-for="(groupItem,groupIndex) in item.drugGroups">
                                <div>
                                    <span v-if="groupIndex === 0" class="t-grey-666">药品组：</span>
                                    <span v-else class="m-l-20">或 </span>
                                    <template v-for="(tagItem,tagIndex) in groupItem">
                                        <a-tag :key="tagItem.code">
                                            {{tagItem.name}}({{tagItem.code}})
        ...
        <medical-clinical-indication-group2 ref="modalForm" @close="UpdateRuleGroup"></medical-clinical-indication-group2>
    </div>
</template>
...
<script>
  ...
  import MedicalClinicalIndicationGroup2 from "@/pages/case/plugin/MedicalClinicalIndicationGroup2"
  import { mixin } from '@/utils/mixin'
  import pick from 'lodash/pick'

  export default {
    name: 'IndicationSetGroup2',
    mixins: [mixin],
    components: { MedicalClinicalIndicationGroup2 },
        ...
        handler(val) {
          ...
          if (val && Array.isArray(val) && val.length > 0) {
            ...
            this.ruleGroup = array.map(r => {
              let obj = {
                groupName: r['ext' + 1],
                diseaseGroups: constructGroupValue(r, 2, 'diseaseGroups'),
                treatGroups: constructGroupValue(r, 3, 'treatGroups'),
                drugGroups: constructGroupValue(r, 5, 'drugGroups')
              }

              let testResultStr = r['ext' + 4]
    ...
    methods: {
      loadData(param) {
        ...
        getAction(this.url.queryNameByCodes, param).then(res => {
          if (res.success) {
            let {diseaseGroups: diseaseGroupsMap = {}
            , treatGroups: treatGroupsMap = {}
            , drugGroups: drugGroupsMap = {}
            , treatments: treatmentsMap = {} } = res.result
            this.ruleGroup.forEach(record => {
              ...
              record.drugGroups && record.drugGroups.forEach(group => {
                group.forEach(r => r.name = drugGroupsMap[r.code] || r.code)
              })
      ...
      onChange() {
        ...
        for (let [index,record] of this.ruleGroup.entries()) {
          ...
          // 药品组
          if(record.drugGroups && record.drugGroups.length > 0){
            data.ext5 = record.drugGroups.map(group => {
              return group.map(r => r.code).join(',')
            }).join('|')
          }
