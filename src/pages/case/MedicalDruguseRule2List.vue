<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="download" @click="handleExportExcel(`合理用药配置`)" v-if="hasExport">导出</a-button>
      <a-button type="primary" icon="download" @click="handleExportFile(`合理用药失效明细${moment().format('YYYYMMDDHHmmss')}.xlsx`,undefined, url.exportInvalidXls)" v-if="hasExport">导出失效明细</a-button>
...
<!--</template>-->
    ...
    <!-- 表单区域 -->
    <medical-druguse-rule2-modal ref="modalForm" @ok="modalFormOk"></medical-druguse-rule2-modal>
...
<script>
  import MedicalDruguseRule2Modal from './modules/MedicalDruguseRule2Modal'
    ...
    data() {
      return {
        ...
        url: {
          ...
          importExcelUrl: "/medical/medicalRuleConfig/importExcel?ruleType=DRUGUSE",
    ...
    methods: {
      ...
      handleImportExcel() {
        let param = {
          ...
          actionType: 'add',
          uploadFileName: 'MEDICAL_DRUGUSE_RULE2.xlsx'
        }
        this.$refs.importModel.loadData(param);
      },
      gotoDrugView(ruleId) {
        this.$router.push({ path: '/review/reviewSystemCommonView', query: { ruleId, ruleType: 'DRUGUSE' } })
      }
    }
  }
</script>
