<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="24">
          <a-col :span="6">
            <a-form-item label="规则类别">
              <m-dict-select-tag :dictCode="ruleLimitDictCode" v-model="queryParam.ruleLimit"
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="download" @click="handleExportExcel(`药品合规配置`)" v-if="hasExport">导出</a-button>
      <a-button type="primary" icon="download"
                @click="handleExportFile(`药品合规失效明细${moment().format('YYYYMMDDHHmmss')}.xlsx`,undefined, url.exportInvalidXls)"
                v-if="hasExport">导出失效明细
      </a-button>
...
<!--</template>-->
    ...
    <!-- 表单区域 -->
    <medical-drug-rule2-modal ref="modalForm" @ok="modalFormOk"></medical-drug-rule2-modal>
...
<script>
import MedicalDrugRule2Modal from './modules/MedicalDrugRule2Modal'
  ...
  data() {
    return {
      ...
      url: {
        ...
        importExcelUrl: '/medical/medicalRuleConfig/importExcel?ruleType=DRUG',
      ...
      fixedParam: { ruleType: 'DRUG' },
      runStatusDict: [],
      ruleLimitMap: {},
      ruleLimitDictCode: 'RULE_LIMIT_DRUG',
      timer: undefined
    }
  },
  beforeMount() {
  ...
  methods: {
    ...
    handleImportExcel() {
      let param = {
        ...
        actionType: 'add',
        uploadFileName: 'MEDICAL_DRUG_RULE2.xlsx'
      }
      this.$refs.importModel.loadData(param)
    },
    gotoDrugView(ruleId) {
      this.$router.push({ path: '/review/reviewSystemCommonView', query: { ruleId, ruleType: 'DRUG2' } })
    }
  }
}
</script>
