 ...
 */
 ...
 export const JeecgListMixin = {
   ...
   computed: {
     ...
     scrollPageY(){
       return this.$store.getters.height - 300
     }
   },
   methods: {
     loadData(arg) {
     ...
     onItemSelect(record, selected) {
       ...
       console.log('onItemSelect', record, selected)
       if (selected) {
     ...
     onSelectAll(selected, selectedRows, changeRows) {
       ...
       if (selected) {
         ...
         this.selectedRowRecords = this.selectedRowRecords.filter(record =>
           !changeRows.some(r => r[id] === record[id]))
       }
     },
     onClearSelected() {
     ...
     resizeableTitle(h, props, children) {
       ...
       // const infoTip = col && col.desc? <a-tooltip title={col.desc}>children</a-tooltip> : undefined
       col && col.desc && (children = <a-tooltip title={col.desc}>{children}</a-tooltip>)
