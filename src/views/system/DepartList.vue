<template xmlns:background-color="http://www.w3.org/1999/xhtml">
  <a-row :gutter="10">
    ...
    <!--导入-->
    <import-modal ref="importModel" @ok="loadTree"></import-modal>
  </a-row>
</template>
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        url: {
          ...
          importExcelNewUrl: "sys/sysDepart/importExcelNew",
        },
    ...
    methods: {
      ...
      setValuesToForm(record) {
        ...
        //省市区赋值
        this.defaultArea = [];
        this.defaultArea.push(record.province);
        this.defaultArea.push(record.city);
        this.defaultArea.push(record.county);
        record.defaultArea = this.defaultArea;
        //解析分析模板名称
        //console.log('record.templateIds',record.templateIds,record.roleType)
        if(record.templateIds){
          if(typeof(record.templateIds)=='string')
            record.templateIds = record.templateIds.split(',');
        }else{
          record.templateIds = []
        }
        if(record.roleType){
          if(typeof(record.roleType)=='string')
            record.roleType = record.roleType.split(',');
        }else{
          record.roleType = []
        }
        // console.log('this.defaultArea-----',record)
        this.form.getFieldDecorator('fax', {initialValue: ''})
        this.form.setFieldsValue(pick(record, 'defaultArea','departName','orgCategory', 'orgCode', 'departOrder', 'mobile', 'fax', 'address', 'memo', 'roleType', 'templateIds'))
      },
      ...
      submitCurrForm() {
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            //解析分析模板名称
            console.log('record.templateIds',values.templateIds,values.roleType)
            if(values.templateIds){
              formData.templateIds = values.templateIds.join(',');
            }
            if(values.roleType){
              formData.roleType = values.roleType.join(',');
            }
     ...
     // <!---- author:os_chengtgen -- date:20190827 --  for:切换父子勾选模式 =======------>
      ...
      // <!---- author:os_chengtgen -- date:20190827 --  for:切换父子勾选模式 =======------>
      onChange(value) {
        console.log('1111111111111',this.defaultArea)
        console.log('----------------',value);
      },
      //导入
      handleImportExcel() {
        let param = {
          title: '部门信息导入',
          importUrl: this.url.importExcelNewUrl,
          actionType: 'add',
          uploadFileName: 'SYS_DEPART.xlsx'
        }
