      ...
      <a-form :form="form">
        ...
        <a-form-item
          ...
          <multi v-slot="slotObj"
                 v-decorator="['apiAuth', {}]"
                 separator="," :disabled="disableSubmit" event="input">
            <a-input placeholder="请输入如：/sys/user,/sys/*" :value="slotObj.value" v-on="slotObj.on" :readOnly="disableSubmit"/>
          </multi>
        </a-form-item>
        <a-form-item
          v-show="!show"
        ...
        <a-form-item
          ...
          :wrapperCol="wrapperCol"
          label="所属系统">
          <m-multi-select-tag placeholder="请选择所属系统(空为适用所有系统)" dictCode="SYSTEM_CODE" :required="false" v-decorator="['systemCodes', {}]"></m-multi-select-tag>
        </a-form-item>
...
<script>
  ...
  export default {
    ...
    components: {Icons,JDictSelectTag, Multi,MMultiSelectTag},
    data () {
      return {
        drawerWidth:700,
        treeData:[],
     ...
     /* add () {
      ...
      add (record) {
        ...
        if (record) {
          ...
          //默认值
          newRecord = {status:'1',permsType:'1',route:true}
        }
        this.edit(newRecord);
      },
      edit (record) {
        ...
        if(this.model.systemCodes){
          this.systemCodes = this.model.systemCodes.split(',')
        }
        //----------------------------------------------------------------------------------------------

        this.visible = true;
        this.loadTree();
        let fieldsVal = pick(this.model,'name','perms','permsType','component','url','sortNo','menuType','status', 'redirect', 'apiAuth', 'systemCodes');
        this.$nextTick(() => {
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            this.model.systemCodes =  this.systemCodes.length>0?this.systemCodes.join(","):''
