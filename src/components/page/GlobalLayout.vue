<template>
  <a-layout class="layout" :class="[device]">
    ...
    <global-header
            ...
            @freshtab="freshtab"
            @toggle="toggle"
            v-show="!headerBarFixed"
    >
      <!--<slot name="header" v-if="layoutMode === 'sidemenu'"></slot>-->
    </global-header>
    ...
    <a-layout
      ...
      <a-layout :style="{ paddingLeft: fixSiderbar && isDesktop() ? `${sidebarOpened ? 200 : 80}px` : '0' }">
      <!-- layout content -->
      <a-layout-content ref="contentRef"
        :style="{overflow:isIframe?'hidden':'auto' }" style="height: 100%;" @scroll="handleScroll">
        <div style="min-height: 100%" :style="{padding: isIframe?0:'12px 24px 0'}" v-if="layoutMode !== 'sidemenu'">
          <slot name="header" v-if="layoutMode !== 'sidemenu'"></slot>
          <slot></slot>
        </div>
...
<script>
  ...
  export default {
    ...
    provide() {
      return {
        getContainer: () => this.$refs.contentRef.$el
      }
    },
    computed: {
      ...mapState({
    ...
    watch: {
      ...
      permissionMenuList: {
        immediate: true,
        handler(val, oldVal) {
          console.log('menus更新', val, oldVal)
          if (val && val.length > 0) {
            this.menus = val
          }
        }
    ...
    beforeMount(){
      this.$bus.$on('scrollTop', val => {
        this.$refs.contentRef.$el.scrollTop = val
      })
    },
    beforeDestroy(){
