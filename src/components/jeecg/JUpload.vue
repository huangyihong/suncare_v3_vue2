...
<script>
  ...
  export default {
    ...
    data(){
      return {
        uploadAction:this.$config['domianURL']+"/sys/common/upload",
    ...
    methods:{
     ...
     handleChange(info) {
        if(info.fileList.length>this.maxUploadFileCount){
          this.$message.warning('最多只能上传' + this.maxUploadFileCount + '个附件！');
          return false;
        }
        ...
        if(info.file.status==='done'){
          if(info.file.response.success){
            ...
            fileList = fileList.filter((file) => {
              if(file.response.status && file.response.status=='history'){
                return true;
              }
       ...
       this.fileList = fileList
        ...
        if(info.file.status==='done' || info.file.status === 'removed'){
          console.log("--change--" +JSON.stringify (fileList))
