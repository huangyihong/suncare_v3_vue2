<template>
  ...
  >
    ...
    <a-select-option v-for="d in options" :key="d.code" :value="d.code">{{ d.value }}</a-select-option>
  </a-select>
      ...
      initSelectValue(){
        if(this.async){
          if(!this.selectedAsyncValue || !this.selectedAsyncValue.key || this.selectedAsyncValue.key!=this.value){
            console.log("这才请求后台")
            queryValByTypeKey({type: this.dict, code :this.value}).then(res=>{
              if(res.success){
                let obj = {
                  key:this.value,
                  label:res.result
      ...
      initDictData(){
        if(!this.async){
          ...
          if(this.dictOptions && this.dictOptions.length>0){
            ...
            //根据字典Code, 初始化字典数组
            querySysDictByType({type: this.dict}).then((res) => {
              if (res.success) {
                this.options = res.result;
              }
            })
